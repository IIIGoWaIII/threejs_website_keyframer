...
`;
  var ModelStory = class _ModelStory {
    constructor() {
      this.loadTimer = null;
      this.state = "idle";
      ...
      this.reduced = false;
      this.isMobile = false;
      this.madeContainerRelative = false;
      this.canvas = null;
      ...
      this.envLightness = 1;
      this.envTint = "#ffffff";
      this.lastEnvLight = null;
      this.lastEnvColor = null;
      this.c1 = null;
      this.c2 = null;
      this.c3 = null;
      this.keyframes = [];
      ...
      this.lightIcons = {};
      this.lightIconVec = null;
      this.kfJumpGuard = false;
      this.kfJumpGuardTimer = null;
      this.freeNav = false;
      this.navTarget = null;
      this.navAz = 0;
      this.navEl = 0;
      ...
      this.navPointerDown = (e) => {
        ...
        e.stopPropagation();
        e.preventDefault();
        this.navDrag = { mode, x: e.clientX, y: e.clientY, pointerId: e.pointerId };
        this.freeNav = true;
        this.camViewLock = false;
        try {
      ...
      this.navPointerMove = (e) => {
        ...
        e.preventDefault();
        if (drag.mode === "orbit") {
          const sens = this.three.MathUtils.degToRad(0.4);
          this.navAz -= sens * dx;
          this.navEl += sens * dy;
      ...
      this.navWheel = (e) => {
        ...
        this.navDist *= Math.pow(1.2, d / 100);
        this.clampNavDist();
        this.freeNav = true;
        this.camViewLock = false;
      };
    ...
    static async create(options) {
      ...
      const instance = new _ModelStory();
    ...
    normalizeOptions(options) {
      return {
        ...
        editor: options.editor === void 0 ? "auto" : options.editor,
        progressMode: options.progressMode === "manual" ? "manual" : "scroll",
        accentColor: options.accentColor || "#fa7c1d",
        loader: options.loader === void 0 ? true : !!options.loader,
        disableOn: {
          mobile: !options.disableOn || options.disableOn.mobile === void 0 ? true : !!options.disableOn.mobile,
          reducedMotion: !options.disableOn || options.disableOn.reducedMotion === void 0 ? true : !!options.disableOn.reducedMotion
        },
        spin: options.spin === void 0 ? true : !!options.spin,
        camera: {
    ...
    initViewer() {
      ...
      this.camera = new this.three.PerspectiveCamera(this.opts.camera.fov, 1, 0.1, 200);
      this.resize();
      const lightDefs = [
        { name: "key", color: 16773600, intensity: 1.7, position: [5, 9, 4], angleDeg: 38, penumbra: 0.3, distance: 0, decay: 1.5 },
        { name: "fill", color: 10336511, intensity: 0.45, position: [-6, 5, -6], angleDeg: 42, penumbra: 0.4, distance: 0, decay: 1.5 },
        { name: "rim", color: 16758903, intensity: 0.5, position: [-3, 2, -8], angleDeg: 40, penumbra: 0.35, distance: 0, decay: 1.5 }
      ];
      lightDefs.forEach((d) => {
        const o = this.opts.lights[d.name] || {};
        const color = o.color !== void 0 ? o.color : d.color;
        const intensity = o.intensity !== void 0 ? o.intensity : d.intensity;
        ...
        L.angle = this.three.MathUtils.degToRad(o.angleDeg !== void 0 ? o.angleDeg : d.angleDeg);
        L.penumbra = o.penumbra !== void 0 ? o.penumbra : d.penumbra;
        L.distance = o.distance !== void 0 ? o.distance : d.distance;
        L.decay = o.decay !== void 0 ? o.decay : d.decay;
    ...
    showLoading() {
      ...
      if (!this.loaderEl) {
        ...
        this.loaderBar = this.loaderEl.querySelector(".ms3d-loader-bar");
        this.loaderPct = this.loaderEl.querySelector(".ms3d-loader-pct");
      }
    ...
    applyProgress() {
      ...
      if (model && !this.editorOpen) {
        if (this.keyframes.length) {
          ...
          const sway = this.progress < 0.22 ? Math.sin(now * 11e-4) * 0.06 : 0;
    ...
    applyEnvironment() {
      ...
      env.traverse((o) => {
        ...
        const pl = o;
        if (pl.isPointLight) {
          pl.intensity *= this.envLightness;
          pl.color.copy(tint);
        }
      });
    ...
    syncEnvControls() {
      const sl = document.getElementById("ms3d-env-light");
      const val = document.getElementById("ms3d-env-light-val");
      const col = document.getElementById("ms3d-env-color");
      if (sl) sl.value = String(Math.round(this.envLightness * 100) / 100);
      if (val) val.value = String(Math.round(this.envLightness * 1e3) / 1e3);
    ...
    kfLerpColor(ha, hb, t) {
      this.c1.set(ha);
      this.c2.set(hb);
      this.c3.copy(this.c1).lerp(this.c2, t);
    ...
    // -------------------------------------------------------------------------
    parseKeyframesList(list, onErr) {
      const out = [];
      for (let i = 0; i < list.length; i++) {
        const k = list[i];
        if (!k || typeof k.t !== "number") {
          if (onErr) onErr("Keyframe " + i + " has no numeric t");
          return null;
        }
        ...
        for (const n in k.lights || {}) {
          const l = k.lights[n] || {};
          const lp = l.pos || {};
          const lt = l.target ? { x: l.target.x || 0, y: l.target.y || 0, z: l.target.z || 0 } : pos.x !== void 0 ? { x: pos.x || 0, y: pos.y || 0, z: pos.z || 0 } : { x: 0, y: 0, z: 0 };
          kf.lights[n] = {
    ...
    loadKeyframes() {
      ...
      if (typeof this.opts.keyframes === "string") {
        fetch(this.opts.keyframes).then((r) => r.json()).then((data) => this.handleKfData(data)).catch(() => {
        });
      } else {
    ...
    restoreSnapshot(s) {
      ...
      m.rotation.fromArray(s.model.rot);
      ...
      this.activeKf = s.activeKf !== void 0 && s.activeKf < this.keyframes.length ? s.activeKf : -1;
      if (s.env) {
    ...
    resetNavState() {
      ...
      if (this.camera && this.camTarget && this.navTarget) {
        ...
        if (pose && this.navPose) {
          ...
          this.navDist = Math.max(this.navPose.length(), 1e-3);
    ...
    seedAuthoredCam(force) {
      ...
      if (pose) {
        ...
        this.camEditDist = Math.max(this.authoredCam.position.distanceTo(pose.camTarget), 1e-3);
      } else if (force && this.camera) {
        this.authoredCam.position.copy(this.camera.position);
        const look = this.camLookTarget || this.camTarget;
        if (look) {
          this.authoredCam.lookAt(look);
          this.camEditDist = Math.max(this.authoredCam.position.distanceTo(look), 1e-3);
        }
      }
    ...
    seedNavFromAuthoredCam(noDolly) {
      ...
      const len = Math.max(this.navPose.length(), 1e-3);
      this.navDist = len;
      this.navAz = Math.atan2(this.navPose.x, this.navPose.z);
    ...
    initEditor() {
      ...
      shelf.innerHTML = [
        ...
        '<div class="ms3d-row"><span class="ms3d-row-label">Light</span><input id="ms3d-env-light" type="range" min="0" max="3" step="0.05" value="1" class="ms3d-input-range"><input id="ms3d-env-light-val" type="number" step="0.05" min="0" max="3" value="1" class="ms3d-input-num"></div>',
        '<div class="ms3d-row"><span class="ms3d-row-label">Color</span><input id="ms3d-env-color" type="color" value="#ffffff" class="ms3d-input-color-wide"></div>',
        "</div>"
      ].join("");
      ...
      helpModal.innerHTML = '<div class="ms3d-modal-card"><div class="ms3d-modal-head"><div class="ms3d-modal-title">Instructions</div><button class="ms3d-modal-close" id="ms3d-help-close">Close</button></div><div class="ms3d-help-modal-body">Scroll to a progress \u2192 position model/lights/camera \u2192 K. For the camera, select <b>Camera</b> \u2014 a camera object appears with a gizmo: drag/rotate it or edit its fields to aim, Numpad0 looks through it, K pins the pose (playback moves the camera between pinned keyframes). Click a diamond or drag the timeline to scrub. MMB/Alt+drag orbit \xB7 Shift+MMB pan \xB7 Wheel zoom \xB7 Numpad1/3/7 front/right/top views (Ctrl=opposite) \xB7 Numpad5 perspective/ortho \xB7 Numpad0 camera view.</div></div>';
      ...
      wrap.appendChild(tl);
      this.diamondsEl = document.getElementById("ms3d-diamonds");
      this.playheadEl = document.getElementById("ms3d-playhead");
      this.attachPlayheadScrub(tl.querySelector(".ms3d-tl-stage"));
      ...
      this.exportText = document.getElementById("ms3d-export-text");
      ...
      this.importText = document.getElementById("ms3d-import-text");
      ...
      const envLightEl = document.getElementById("ms3d-env-light");
      const envLightValEl = document.getElementById("ms3d-env-light-val");
      const envColorEl = document.getElementById("ms3d-env-color");
      const syncEnv = (el) => {
        if (!el.__pushedUndo) {
          el.__pushedUndo = true;
          this.pushUndo();
        }
      ...
      envLightValEl.addEventListener("input", (e) => {
        syncEnv(e.currentTarget);
      });
      envColorEl.addEventListener("input", (e) => {
        syncEnv(e.currentTarget);
      });
      [envLightEl, envLightValEl, envColorEl].forEach((inp) => {
        inp.addEventListener("focus", (e) => {
          e.currentTarget.__pushedUndo = false;
        });
      });
      ...
      wrap.querySelectorAll(".ms3d-sel").forEach((b) => {
        b.addEventListener("click", () => {
          this.selectObject(b.dataset.sel);
        });
      });
      wrap.querySelectorAll(".ms3d-mode").forEach((b) => {
        b.addEventListener("click", () => {
          if (this.gizmo) this.gizmo.setMode(b.dataset.mode);
        });
      });
      const spaceBtn = document.getElementById("ms3d-space");
      const syncSpaceBtn = () => {
      ...
      gizmo.addEventListener("objectChange", () => {
        ...
        if (this.selType === "camera") {
          ...
          ["key", "fill", "rim"].forEach((n) => {
            if (this.lightsMap[n].target) this.lightsMap[n].target.position.copy(this.model.position);
          });
      ...
      if (!this.camHelper) {
        ...
        const camGrey = new this.three.Color(8421504);
      ...
      if (!this.authorGrid) {
        ...
        const grid = new this.three.GridHelper(gsize, 24, 10132122, 5658198);
        ...
        const axisLines = [
          [new this.three.Vector3(-alen, 0, 0), new this.three.Vector3(alen, 0, 0), 16726832],
          [new this.three.Vector3(0, -alen, 0), new this.three.Vector3(0, alen, 0), 3458905],
          [new this.three.Vector3(0, 0, -alen), new this.three.Vector3(0, 0, alen), 31487]
        ];
        axisLines.forEach((d) => {
    ...
    selectObject(name) {
      ...
      if (this.editorEl) {
        this.editorEl.querySelectorAll(".ms3d-sel").forEach((b) => {
    ...
    updatePanel() {
      ...
      if (this.selType === "model") {
        ...
        bindings.push(["Pos X", () => obj.position.x, (v) => {
          obj.position.x = v;
        }, 0.01]);
        bindings.push(["Pos Y", () => obj.position.y, (v) => {
          obj.position.y = v;
        }, 0.01]);
        bindings.push(["Pos Z", () => obj.position.z, (v) => {
          obj.position.z = v;
        }, 0.01]);
        bindings.push(["Rot X \xB0", () => this.three.MathUtils.radToDeg(obj.rotation.x), (v) => {
          obj.rotation.x = this.three.MathUtils.degToRad(v);
        }, 0.2]);
        bindings.push(["Rot Y \xB0", () => this.three.MathUtils.radToDeg(obj.rotation.y), (v) => {
          obj.rotation.y = this.three.MathUtils.degToRad(v);
        }, 0.2]);
        bindings.push(["Rot Z \xB0", () => this.three.MathUtils.radToDeg(obj.rotation.z), (v) => {
          obj.rotation.z = this.three.MathUtils.degToRad(v);
        }, 0.2]);
      } else if (this.selType === "camera") {
        const cam = this.authoredCam;
        bindings.push(["Pos X", () => cam.position.x, (v) => {
          cam.position.x = v;
        }, 0.01]);
        bindings.push(["Pos Y", () => cam.position.y, (v) => {
          cam.position.y = v;
        }, 0.01]);
        bindings.push(["Pos Z", () => cam.position.z, (v) => {
          cam.position.z = v;
        }, 0.01]);
        bindings.push(["Target X", () => this.authoredCamLookTarget().x, (v) => {
          const t = this.authoredCamLookTarget();
          t.x = v;
          cam.lookAt(t);
          this.camEditDist = Math.max(cam.position.distanceTo(t), 1e-3);
        }, 0.01]);
        bindings.push(["Target Y", () => this.authoredCamLookTarget().y, (v) => {
          const t = this.authoredCamLookTarget();
          t.y = v;
          cam.lookAt(t);
          this.camEditDist = Math.max(cam.position.distanceTo(t), 1e-3);
        }, 0.01]);
        bindings.push(["Target Z", () => this.authoredCamLookTarget().z, (v) => {
          const t = this.authoredCamLookTarget();
          t.z = v;
          cam.lookAt(t);
          this.camEditDist = Math.max(cam.position.distanceTo(t), 1e-3);
        ...
        bindings.push(["Intensity", () => obj.intensity, (v) => {
          obj.intensity = v;
        }, 5e-3]);
        bindings.push(["Angle \xB0", () => this.three.MathUtils.radToDeg(obj.angle), (v) => {
          obj.angle = this.three.MathUtils.degToRad(v);
          this.refreshLightShadow(obj);
        }, 0.2]);
        bindings.push(["Penumbra", () => obj.penumbra, (v) => {
          obj.penumbra = v;
        }, 0.01]);
        bindings.push(["Distance", () => obj.distance, (v) => {
          obj.distance = Math.max(0, v);
          this.refreshLightShadow(obj);
        }, 0.05]);
        bindings.push(["Decay", () => obj.decay, (v) => {
          obj.decay = v;
        }, 0.01]);
        bindings.push(["Target X", () => obj.target ? obj.target.position.x : 0, (v) => {
          if (obj.target) {
            obj.target.position.x = v;
            this.refreshLightShadow(obj);
          }
        }, 0.01]);
        bindings.push(["Target Y", () => obj.target ? obj.target.position.y : 0, (v) => {
          if (obj.target) {
            obj.target.position.y = v;
            this.refreshLightShadow(obj);
          }
        }, 0.01]);
        bindings.push(["Target Z", () => obj.target ? obj.target.position.z : 0, (v) => {
          if (obj.target) {
            obj.target.position.z = v;
            this.refreshLightShadow(obj);
          }
        }, 0.01]);
      }
      ...
      bindings.forEach((b) => {
        ...
        inp.addEventListener("change", (e) => {
          e.currentTarget.value = b[1]().toFixed(3);
        });
    ...
    addKeyframe() {
      ...
      if (this.selType === "camera") {
        for (let i = 0; i < this.keyframes.length; i++) {
          if (Math.abs(this.keyframes[i].t - this.progress) < 5e-4) {
    ...
    selectKeyframe(i) {
      ...
      this.updatePanel();
      this.updatePlayhead();
      this.kfJumpGuardTimer = setTimeout(() => {
        this.kfJumpGuard = false;
      }, 300);
    }
    ...
    importKeyframes() {
      ...
      if (!raw) {
        err.textContent = "Paste JSON first";
        return;
      }
      let data;
      try {
        ...
        err.textContent = "Invalid JSON: " + (e.message || String(e));
        return;
      }
      const list = Array.isArray(data) ? data : data && Array.isArray(data.keyframes) ? data.keyframes : null;
      if (!list) {
        err.textContent = "Expected an array or { keyframes: [...] }";
        return;
      }
      const out = this.parseKeyframesList(list, (msg) => {
        err.textContent = msg;
      });
      if (!out) return;
      this.pushUndo();
